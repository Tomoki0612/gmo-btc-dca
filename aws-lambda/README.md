...
「関数の作成」をクリック
2. コードのアップロード
lambda_function.py の内容をコピー
Lambdaコンソールのコードエディタに貼り付け
「Deploy」をクリック
...
Lambda関数 > レイヤー > レイヤーの追加
「ARNを指定」を選択
以下のARNを入力（東京リージョンの場合）:
...
ターゲット: Lambda関数 > btc-auto-purchase
作成
方法2: AWS CLI（上級者向け）
1. デプロイパッケージの作成
bash
...
よくあるエラー
エラー: "Unable to import module 'lambda_function'" → requestsライブラリが見つからない。レイヤーを追加してください。

エラー: "Task timed out after 3.00 seconds" → タイムアウト設定を30秒に増やしてください。

エラー: "GMO_API_KEY が環境変数に設定されていません" → 環境変数が正しく設定されているか確認してください。
